...
class TestService:
    ...
    def send_test_invitations(self, test_id: int, test_link: str = None) -> Dict:
        ...
        """
        # Removed credential check since we use microservice
    ...
    def _send_test_email(self, candidate_email: str, candidate_name: str, test_link: str) -> bool:
        """
        Send test invitation email to a candidate via Notification Service
        """
        try:
            NOTIFICATION_URL = os.getenv("NOTIFICATION_SERVICE_URL", "http://localhost:5005")
            payload = {
                "candidate_email": candidate_email,
                "candidate_name": candidate_name,
                "test_link": test_link
            }
            resp = requests.post(f"{NOTIFICATION_URL}/send/test-invitation", json=payload, timeout=5)
            if resp.status_code == 200 and resp.json().get("success"):
                return True
            else:
                print(f"Notification Service error: {resp.text}")
                return False
            
        except Exception as e:
            print(f"Error calling Notification Service for {candidate_email}: {str(e)}")
            return False
