...
const InterviewCandidates = () => {
  ...
  const loadCandidates = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`${API_BASE_URL}/interviews/candidates-with-schedules`);
  ...
  const handleSelect = async (candidateEmail) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/interviews/select-candidate`, {
        method: 'POST',
  ...
  const handleReject = async (candidateEmail) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/interviews/reject-candidate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ candidate_email: candidateEmail })
      });
